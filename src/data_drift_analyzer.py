...
class DataDriftAnalyzer:
    def __init__(self, train_df: pl.DataFrame, inf_df: pl.DataFrame, 
                 join_keys: List[str] = ['correlation_id', 'hotel_id', 'rate_plan_code'],
        ...
        self.common_cols = [
            col for col in train_df.columns if col in inf_df.columns and col not in join_keys and col not in exclude_cols
        ]
        
        # Identify numeric columns using polars selectors
        self.numeric_cols = [col for col in self.common_cols 
                           if col in train_df.select(cs.numeric()).columns]
        
        # Remaining columns are categorical
        self.categorical_cols = [col for col in self.common_cols 
                               if col not in self.numeric_cols]
        
        print(f"Analyzing {len(self.numeric_cols)} numeric columns: {self.numeric_cols}")
        print(f"Analyzing {len(self.categorical_cols)} categorical columns: {self.categorical_cols}")
    ...
    def analyze_distributions(self, data1: pl.DataFrame, data2: pl.DataFrame, 
                            col: str) -> Dict:
        ...
        if col in self.numeric_cols:
            ...
            val_counts1 = (data1.select(pl.col(col))
                        .group_by(col)
                        .agg(pl.len().alias("counts"))
                        .sort("counts", descending=True))
            
            val_counts2 = (data2.select(pl.col(col))
                        .group_by(col)
                        .agg(pl.len().alias("counts"))
                        .sort("counts", descending=True))
            
            # Get categories that appear in both datasets
            common_categories = set(val_counts1[col].to_list()) & set(val_counts2[col].to_list())
            ...
            if not common_categories:
                ...
                if sum(counts1) > 0 and sum(counts2) > 0:
                    try:
                        chi2_stat, p_value = stats.chi2_contingency([counts1, counts2])[:2]
    ...
    def print_drift_summary(self, results: Dict, analysis_type: str = ""):
        ...
        print("=" * 80)
        print(f"Sample sizes: Training: {results['sample_sizes']['training']}, "
        ...
        for col, analysis in results['column_analysis'].items():
            if analysis['type'] == 'numeric':
                ...
                if analysis['p_value'] < 0.05:
                    ...
                    print(f"  - Unique values difference: {analysis['unique_values_diff']}")
                    if analysis.get('warning'):
                        print(f"  - Warning: {analysis['warning']}")
                    if analysis['categories_only_in_train']:
                        print(f"  - Categories only in training: {analysis['categories_only_in_train']}")
                    if analysis['categories_only_in_inference']:
                        print(f"  - Categories only in inference: {analysis['categories_only_in_inference']}")


    def plot_distribution_comparison(self, data1: pl.DataFrame, data2: pl.DataFrame, 
                                col: str, title_prefix: str = ''):
        ...
        if col in self.numeric_cols:
            ...
            if len(data1_array) > 0 and len(data2_array) > 0:
                ...
                if var1 == 0 and var2 == 0:
                    ...
                    bins = max(10, min(50, int(np.sqrt(len(data1_array) + len(data2_array)))))
                    
                    if var1 == 0:
                        constant_val = data1_array[0]
                        plt.axvline(x=constant_val, color='blue', linestyle='-', 
                                label=f'Training (constant={constant_val:.2f})')
                        plt.hist(data2_array, bins=bins, alpha=0.5, 
                            density=True, label='Inference', color='orange')
                    else:
                        constant_val = data2_array[0]
                        plt.hist(data1_array, bins=bins, alpha=0.5, 
                            density=True, label='Training', color='blue')
                        plt.axvline(x=constant_val, color='orange', linestyle='-', 
                    ...
                    try:
                        # Try KDE first
                        sns.kdeplot(data=data1_array, label='Training', alpha=0.5, warn_singular=False)
                        sns.kdeplot(data=data2_array, label='Inference', alpha=0.5, warn_singular=False)
                ...
                plt.text(0.5, 0.5, 'Insufficient data for distribution plot\n(too many null values)', 
                        ha='center', va='center')
            ...
            all_categories = set(val_counts1[col].to_list()) | set(val_counts2[col].to_list())
            all_categories = sorted(list(all_categories))[:10]  # Limit to top 10 categories
            ...
            if len(all_categories) > 0:
                ...
                for category in all_categories:
                    ...
                    count1 = val_counts1.filter(pl.col(col) == category)['counts'].to_list()
                    counts1.append(count1[0] if count1 else 0)
                    count2 = val_counts2.filter(pl.col(col) == category)['counts'].to_list()
    ...
    def analyze_drift(self) -> Tuple[Dict, Dict]:
        ...
        try:
            joined_train = joined_train.join(
                joined_inf.select(self.join_keys), 
                on=self.join_keys,
                how='inner'
            )

            joined_inf = joined_inf.join(
                joined_train.select(self.join_keys), 
                on=self.join_keys,
                how='inner'
            )
            ...
            for col in self.common_cols:
                try:
                    ...
                    self.plot_distribution_comparison(
                        joined_train, joined_inf, col, 
                        title_prefix='Joined Data: '
                    )
                    
                    # Overall analysis
                    overall_results['column_analysis'][col] = self.analyze_distributions(
                        self.train_df, self.inf_df, col
                    )
                    self.plot_distribution_comparison(
                        self.train_df, self.inf_df, col, 
                        title_prefix='Overall Data: '
                    )
